...
def ask_json(
    ...
    *,
    ...
    for attempt in range(3):
        try:
            response = completion(**_call_kwargs(model, messages, temperature, current_max_tokens))
        ...
        if choice.finish_reason == "length":
            if attempt < 2:
                ...
                print(
                    f"WARNING: finish_reason='length' (response truncated); "
                    f"retrying with max_tokens={current_max_tokens}.",
                    file=sys.stderr,
                )
                continue
            raise RuntimeError(
                f"LLM response was truncated (finish_reason='length') after "
                f"{attempt + 1} attempts, even at max_tokens={current_max_tokens}. "
