...
def _resolve_fit_section(report: dict) -> tuple[dict, str]:
    """
    Pick whichever fit-audit key the report actually has.

    Prefers "degree_alignment" (app.py's pipeline: dimension /
    degree_curricular_focus / jd_requirement / match_level / rationale,
    scored as degree_alignment_score) and falls back to "background_fit"
    (main.py's pipeline: dimension / resume_value / jd_requirement /
    match_level / rationale, scored as background_fit_score). Returns the
    section dict plus a mode string used to pick field names/labels.
    """
    if "degree_alignment" in report:
        return report["degree_alignment"], "degree_alignment"
    if "background_fit" in report:
        return report["background_fit"], "background_fit"
    return {}, "degree_alignment"
...
def _build_lines(report: dict) -> list[str]:
    ...
    fit, fit_mode = _resolve_fit_section(report)
    ...
    # 7. Degree alignment / background fit
    fit_checks = fit.get("checks", [])
    if fit_mode == "degree_alignment":
        section_title = "Degree Alignment"
        focus_col     = "Degree Focus"
        focus_field   = "degree_curricular_focus"
        score_key     = "degree_alignment_score"
        degree_name   = fit.get("degree", meta.get("degree", ""))
        lead_line     = f"**Degree:** {degree_name}  "
    else:
        section_title = "Background Fit"
        focus_col     = "Résumé Value"
        focus_field   = "resume_value"
        score_key     = "background_fit_score"
        lead_line     = None

    fit_score = fit.get(score_key, 0)
    ...
    lines += [
        ...
        "",
        f"| Dimension | {focus_col} | JD Requirement | Match | Rationale |",
        "|---|---|---|---|---|",
    ]
    if fit_checks:
        for c in fit_checks:
            lines.append(
                f"| {c.get('dimension', '')} "
                f"| {c.get(focus_field) or '—'} "
                f"| {c.get('jd_requirement') or '—'} "
                f"| {c.get('match_level', '')} "
                f"| {c.get('rationale', '')} |"
            )
    else:
        lines.append(f"| — | — | — | — | No {section_title.lower()} checks returned. |")
    lines.append("")
    ...
    bg_contrib  = round(fit_score * 0.10, 1)
    fit_label   = "Degree alignment" if fit_mode == "degree_alignment" else "Background fit"
    lines += [
        "## Score Breakdown",
        "",
        ...
        f"| {fit_label:<16} | {fit_score} | 10% | {bg_contrib} |",
        f"| **Total**        |     |     | **{score}** |",
        "",
    ]
