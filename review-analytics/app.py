...
with st.sidebar:
    ...
    for label, db_category in categories.items():
        with st.expander(label):
            records = db_manager.get_summaries_by_category(db_category)
            if not records:
...
if st.session_state.view_mode == "view_past" and st.session_state.selected_summary_id is not None:
    record = db_manager.get_summary_by_id(st.session_state.selected_summary_id)
    if record:
        ...
        try:
            ...
            if st.button("🚀 Execute Sentiment Analysis", use_container_width=True):
                with st.spinner("Gemini is interpreting text blocks and mapping score parameters..."):
                    try:
                        clean_summary, rating, category = gemini_service.analyze_review_sentiment(
                            review_text
                        )

                        # COMMIT INSIGHTS TO PERMANENT DISK STORAGE
                        db_manager.save_summary(uploaded_file.name, clean_summary, rating, category)
